...
CREATE or ALTER  PROCEDURE Admin.sp_Presupuesto_GastoSugerido_vs_GastoMes  
    @NIT NVARCHAR(20),  
    @Anio INT,  
    @Mes INT  
AS  
   ...
   WHERE YEAR(Periodo) = @Anio  
     AND MONTH(Periodo) = @Mes;  
        ...
        SELECT   
            ...
            pg.CodigoProducto,  
            ISNULL(CAST(CASE   
                WHEN @MesColumna = 'Enero' THEN pg.Enero  
                WHEN @MesColumna = 'Febrero' THEN pg.Febrero  
                WHEN @MesColumna = 'Marzo' THEN pg.Marzo  
                WHEN @MesColumna = 'Abril' THEN pg.Abril  
                WHEN @MesColumna = 'Mayo' THEN pg.Mayo  
                WHEN @MesColumna = 'Junio' THEN pg.Junio  
                WHEN @MesColumna = 'Julio' THEN pg.Julio  
                WHEN @MesColumna = 'Agosto' THEN pg.Agosto  
                WHEN @MesColumna = 'Septiembre' THEN pg.Septiembre  
                WHEN @MesColumna = 'Octubre' THEN pg.Octubre  
                WHEN @MesColumna = 'Noviembre' THEN pg.Noviembre  
                WHEN @MesColumna = 'Diciembre' THEN pg.Diciembre  
        ...
        LEFT JOIN Admin.PresupuestoGasto pg  
				ON dc.CodigoCuenta = pg.CodigoProducto  
        WHERE dc.ArchivoID=@ARCHIVOID  
				AND pg.AnioPresupuesto = @Anio  
				AND pg.NIT = @NIT;  
